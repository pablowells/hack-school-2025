...
const getPoll = async (req, res) => {
  ...
  const { id } = req.params;
  const poll = await Poll.findById(req.id);
...
const postVote = async ({ req, res }) => {
  ...
  if (pollId || optionId) res.status(400).json({ error: 'Invalid request'}); // update for 400 Error
  ...
  if (updateOption.modifiedCount == 0) return res.status(404); // update for 400 Error
